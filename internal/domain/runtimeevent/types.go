...
// EventKind is the normalized event category extracted from runtime material.
...
// Event is one normalized event extracted from runtime material.
type Event struct {
	Kind       EventKind
	Timestamp  float64
	RawType    string
...
// CompactionEvent is one runtime compaction lifecycle event.
